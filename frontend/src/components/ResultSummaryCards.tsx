import { CheckCircle2, AlertTriangle, Sparkles, Shield, Minus } from "lucide-react";
...
export function ResultSummaryCards({ result }: { result: ReconcileResponse }) {
  ...
  if (!unmatched) {
    const ut = result.unmatched_transactions?.length ?? 0;
    const ut2 = result.unmatched_bank_rows?.length ?? 0;
